...
Проверка что на телефоне есть поддержка Vulkan
```
ls /system/lib64/libvulkan.so
```

Усттановка необходимых библиотек в `Termux`
...
После сборки при запуске должна появиться строчка вроде:
```
ggml_vulkan: Using Mali-G76 | uma: 1 | fp16: 1 | warp size: 16
```
