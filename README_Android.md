...
- [(Дополнительно) Вариант установки и запуска приложения через Ubuntu (Andronix)](#дополнительно-вариант-установки-и-запуска-приложения-через-ubuntu-andronix)
...
- [(Дополнительно) Подключение ПК к телефону по SSH](#дополнительно-подключение-пк-к-телефону-по-ssh)
...
## Запуск Python приложения в Termux
...
## (Дополнительно) Вариант установки и запуска приложения через Ubuntu (Andronix)
...
## (Дополнительно) Подключение ПК к телефону по SSH
...
**5) Узнать имя пользователя и IP адрес телефона Android**
...
**6) Подключение из ПК с телефону через Tabby**
...
- `Порт` 8022
- `Имя пользователя` из предыдущего шага из команды whoami (например `u0_a461`)
...
Если `WinSCP` не открывает телефон (при открытии открывается окошко входа) то нужно настроить `WinSCP` вручную  
Для этого на странице входа (появляется после запуска WinSCP) ввести:
- `Имя хоста` который получили на предыдущем шаге командой ifconfig (например `192.168.43.45`) 
- `Порт` 8022
- `Имя пользователя` из предыдущего шага из команды whoami (например `u0_a461`)
- `Еще` - `Еще` - `SSH` - `Аутентификация` - `Файл закрытого ключа` - выбрать файл закрытого ключа SSH (например `C:\Users\ИМЯ_ПОЛЬЗОВАТЕЛЯ\.ssh\id_ed25519_phone`) - при этом нужно сначала чтобы отображались все ключи справа внизу, затем он запросит конвертацию ключа в формат программы, нажать ОК и закрыть окно доп настроек
- Опционально нажать `Сохранить` для удобного последующего входа и нажать `Войти`
