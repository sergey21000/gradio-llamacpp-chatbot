...
- [Дополнительно](#Дополнительно)
  ...
  - [ Подключение из ПК к телефону по SSH через Tabby + WinSCP](#new-Подключение-из-ПК-к-телефону-по-SSH-через-Tabby-WinSCP)
...
**4) Сборка llama.cpp на Android**

Установка библиотек для сборки в `Termux`
```sh
pkg update
pkg install cmake clang libandroid-spawn
```

Клонирование репозитория llama.cpp в папку проекта
```
...
Для удобства можно добавить путь до `llama-server` в системную директорию `Termux` и вызывать `llama-server` из любого места
```
...
Опциоанльная проверка запуска сервера с моделью - в данном примере модель в формате GGUF находится в папке `llama_cache`
```
...
Инструкции для сборки llama.cpp с поддержкой GPU (OpenCL) находится здесь
https://github.com/ggml-org/llama.cpp/blob/master/docs/build.md#android

**5) Запуск приложения**

Перед запуском открыть файл настроек `.settings.env` и указать путь до скомпилированной на предыдущем этапе llama.cpp
```env
LLAMACPP_DIR=llama.cpp/build/bin
```

переход в папку приложения (выход из текущей папки llama.cpp на уровень выше)
```
cd ..
# /data/data/com.termux/files/home/projects/gradio-llamacpp-chatbot
```
